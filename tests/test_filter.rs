...
use exg::filter::{apply_fir_zero_phase, design_highpass};
use ndarray::Array2;
...
fn filter_coeffs_match_scipy() {
    ...
    assert_eq!(
        h_rust.len(),
        h_ref.len(),
        "tap count mismatch: Rust={} SciPy={}",
        h_rust.len(),
        h_ref.len()
    );

    let h_rust_arr = ndarray::Array::from_vec(h_rust.clone()).into_dyn();
...
fn filter_coeffs_sum_near_zero() {
    ...
    assert!(
        s.abs() < 1e-5,
        "sum(h) = {s:.2e}, expected ≈ 0 for highpass"
    );
}
...
fn filter_application_matches_mne() {
    ...
    assert!(max_err < 1e-4, "max filter error {max_err:.2e} >= 1e-4");
    assert!(
        max_err / sigma < 0.0001,
...
fn filter_removes_sub_hz_content() {
    ...
    assert!(
        rms > 0.5,
        "RMS too low ({rms:.3}), pass-band signal attenuated?"
    );
    assert!(
        rms < 0.85,
        "RMS too high ({rms:.3}), stop-band not attenuated?"
    );
}
