...
use exg::normalize::{baseline_correct_inplace, zscore_global_inplace};
...
fn zscore_matches_numpy() {
    ...
    let mut data =
        Array2::from_shape_vec((shape[0], shape[1]), x_arr.iter().cloned().collect()).unwrap();
...
fn baseline_correction_matches_mne() {
    ...
    let mut epochs =
        Array3::from_shape_vec((n_e, n_c, n_t), x_arr.iter().cloned().collect()).unwrap();
...
fn baseline_per_channel_mean_is_zero() {
    ...
    let s = x_arr.shape();
    let mut epochs =
        Array3::from_shape_vec((s[0], s[1], s[2]), x_arr.iter().cloned().collect()).unwrap();
    baseline_correct_inplace(&mut epochs);
