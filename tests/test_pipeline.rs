...
fn full_pipeline_matches_mne_reference() {
    ...
    assert_eq!(
        epochs.len(),
        n_e,
        "epoch count: Rust={} MNE={n_e}",
        epochs.len()
    );

    // Flatten for comparison.
    let got_flat: Vec<f32> = epochs
        .iter()
    ...
    assert!(max_err < 5e-3, "max abs error {max_err:.2e} >= 5e-3");
    assert!(
        max_err / sigma * 100.0 < 0.5,
...
fn pipeline_step_resample() {
    ...
    assert!(
        max_err / sigma * 100.0 < 0.2,
        "resample step: rel error {:.4}%",
        max_err / sigma * 100.0
    );
}
...
fn pipeline_step_highpass() {
    ...
    let sigma = array_std(y_ref);
    assert!(
        max_err / sigma * 100.0 < 0.01,
        "hp filter step: rel error {:.4}%",
        max_err / sigma * 100.0
    );
}
